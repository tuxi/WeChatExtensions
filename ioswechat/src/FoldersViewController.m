...
//        _fileManager = [OSFileManager defaultManager];
        ...
        FoldersViewController * weakSelf = self;
...
//            [weakSelf copyFiles:selectorFiles toRootDirectory:weakSelf.rootDirectory];
        };
    } else {
...
//        FoldersViewController *strongSelf = weakSelf;
...
- (NSArray<id<UIPreviewActionItem>> *)previewActionItems {
    ...
    BOOL isDirectory;
    BOOL fileExists = [[NSFileManager defaultManager] fileExistsAtPath:self.rootDirectory isDirectory:&isDirectory];
    if (!fileExists && !isDirectory) {
        return nil;
    }

    UIPreviewAction *deleteAction = [UIPreviewAction actionWithTitle:@"Delete" style:UIPreviewActionStyleDefault handler:^(UIPreviewAction * _Nonnull action, UIViewController * _Nonnull previewViewController) {
        [UIAlertView showWithTitle:@"确定删除?" message:nil cancelButtonTitle:@"Cancel" otherButtonTitles:@[@"OK"] tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex) {
            if (buttonIndex == 1) {
                NSError *error = nil;
                [[NSFileManager defaultManager] removeItemAtPath:self.rootDirectory error:&error];
            }
        }];
    }];
...
//        [self copyFiles:@[item] toRootDirectory:NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).firstObject];
    }];
...
- (void)setFileModel:(FileAttributeItem *)fileModel {
    ...
    if (isDirectory) {
        ...
        self.detailTextLabel.text = [NSString stringWithFormat:@"%ld个文件", (unsigned long)fileModel.subFileCount];
